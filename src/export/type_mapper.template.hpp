...
namespace ct {
	namespace TypeMapperTemplate {
		template<typename T>
		auto mapType(ct::TypeMapper &mapper, ct::proto::TypeDefinition &out, typename std::remove_reference<T>::type const &type, int)
		...
		template<typename T>
		void mapType(ct::TypeMapper &mapper, ct::proto::TypeDefinition &out, typename std::remove_reference<T>::type const &type, long) {
			out.mutable_specifier()->set_name(type.getLocallyUnqualifiedSingleStepDesugaredType().getAsString());
		}


		template<typename T>
		void mapType(ct::TypeMapper &mapper, ct::proto::TypeDefinition &out, typename std::remove_reference<T>::type const &type) {
